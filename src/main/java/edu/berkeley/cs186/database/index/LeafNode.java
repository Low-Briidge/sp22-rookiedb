...
//        if (!keys.contains(key)) return getRightSibling().get();
        return this;
    }
...
//        sync();
        // key exists
        if (keys.contains(key)) throw new BPlusTreeException("Key exists!");

        // add entries to the right position of oldNode
        ...
        if (keys.size() > d * 2) {
            ...
            // copy last d+1 entries to newNode
            for (int i = d; i < keys.size(); i++) {
                newKeys.add(keys.get(i));
                newRids.add(rids.get(i));
            }

            // delete last d+1 entries from oldNode
            for (int i = 0; i < d + 1; i++) {
            ...
            LeafNode newLeaf = new LeafNode(this.metadata, this.bufferManager, newKeys,
                    newRids, this.rightSibling, treeContext);
            this.rightSibling = Optional.of(newLeaf.page.getPageNum());
            sync();
            return Optional.of(new Pair<>(newLeaf.keys.get(0), newLeaf.page.getPageNum()));
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int d = metadata.getOrder();
        int maxSize = (int) (d * 2 * fillFactor + 0.5);
        while (maxSize >= keys.size() && data.hasNext()) {
            Optional<Pair<DataBox, RecordId>> input = Optional.of(data.next());
            DataBox key = input.get().getFirst();
            RecordId rid = input.get().getSecond();
            ...
            if (keys.size() > maxSize) {
                ...
                newKeys.add(this.keys.remove(keys.size() - 1));
                newRids.add(this.rids.remove(rids.size() - 1));

                LeafNode newLeaf = new LeafNode(metadata, bufferManager, newKeys, newRids, newRightSibling,
                        treeContext);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> rightSibling = right == -1 ? Optional.empty() : Optional.of(right);
